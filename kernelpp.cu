		 ...
		 Real * dev_rotm, Real * dev_opos)
{
	...
	/*Kernel1: get part of the calculation*/
	/*Get the pixel list, flux, angle and other variables*/
	...
	starttime = clock();
	//thrust :: stable_sort (host_par, host_par + nmax, thrust::greater<MapParticle>());
	//std::cout <<"sort cost: " << (clock() - starttime) / 1000.0 << " secs. "<< std::endl;
	cudaStatus = cudaMemcpy(host_par, dev_par, sizeof(MapParticle) * nmax, cudaMemcpyDeviceToHost);
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMemcpy failed!");
		exit(0);
	...
	/*starttime = clock();
	generate_map_step1<<<dimGrid, dimBlock>>>(Nside, theta0, nmax,
		dev_par, dev_rotm, dev_opos);
	cudaStatus = cudaDeviceSynchronize();
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaDeviceSynchronize returned error code %d after launching addKernel!\n", cudaStatus);
	...
	//std::cout <<"step1 cost: " << (clock() - starttime) / 1000.0 << " secs. "<< std::endl;
	//std::cout << (clock() -starttime) / 1000.0 << " sec -> step1 end" << std::endl;
	/*Copy back the particles, reallocate the memory for it*/
	starttime = clock();
	/*cudaStatus = cudaMemcpy(host_par, dev_par, sizeof(MapParticle) * nmax, cudaMemcpyDeviceToHost);
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMemcpy failed!");
	...
	//std::cout <<"step2 cost: " << (clock() - starttime) / 1000.0 << " secs. "<< std::endl;
	//std::cout << (clock() -starttime) / 1000.0 << " sec -> step2 end" << std::endl;
	//copy the skymapdata back
...
void generate_map_step0(const long Nside, 
	...
	if( density < 0.0){
		...
		particles[i].xpos = floor(ad * ad * ((double)(12L * Nside * Nside)) / 4) + 1;
