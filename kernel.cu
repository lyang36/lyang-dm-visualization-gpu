...
cudaError_t doWithCuda(const long MAX_Num_Paritcle, const long Nside, 
		 const Real theta0, const Real fluxfactor, const long nmax,
		 Real *dev_allskymap, MapParticle * dev_par,
		 Real * dev_rotm, Real * dev_opos)
{
	...
	generate_map<<<dimGrid, dimBlock>>>(Nside, theta0, fluxfactor , nmax, dev_allskymap, 
		dev_par, dev_rotm, dev_opos);
...
void in_ring_allskymap(long nside_, int iz, Real phi0, Real dphi,
  const Real vecx, const Real vecy, const Real vecz, 
  const Real angular_radius, const Real norm, const Real aflux, Real * allskymap){
	long npface_ = nside_ * nside_;
	long ncap_   = (npface_-nside_)<<1;
	long npix_   = 12 * npface_;
//	Real fact2_  = 4. / (Real) npix_;
...
#ifdef POS_FLOAT
			atomicAdd(allskymap + i, _flux * aflux);
#else
			allskymap[i] += _flux * aflux;
			__syncthreads();
#endif
	}
	else
...
#ifdef POS_FLOAT
			atomicAdd(allskymap + pixnum, _flux * aflux);
#else
			allskymap[pixnum] += _flux  * aflux;
			__syncthreads();
#endif
			/*if( ++pos < maxnum){
				listir[(pos)] = pixnum;
...
__device__
void query_disc (long nside_, const Real theta, const Real phi, 
	const Real radius, const int maxlength, int *listpix, int & pos){
	//int pos;
	pos = -1;
	//disknum = 0;
	long npface_ = nside_ * nside_;
	...
	for (int m=1; m<irmin; ++m) // rings completely in the disc
		in_ring (nside_, m, 0, pi, listpix, maxlength, pos);
	...
	for (int iz=irmin; iz<=irmax; ++iz) // rings partially in the disc
	{
		...
		in_ring (nside_, iz, phi, dphi, listpix, maxlength, pos);
	}
...
void query_disc_calcflux(long nside_, const Real theta, const Real phi, const Real radius, 
	const Real vecx,  const Real vecy, const Real vecz, const Real weight_norm, const Real aflux, Real *allmap){
	long npface_ = nside_ * nside_;
//	long ncap_   = (npface_-nside_)<<1;
	long npix_   = 12 * npface_;
	Real fact2_  = 4. / (Real) npix_;
	...
	if (rlat1<=0) // north pole in the disc
	for (int m=1; m<irmin; ++m) // rings completely in the disc
		in_ring_allskymap (nside_, m, 0, pi, vecx, vecy, vecz, radius / 2.0, weight_norm, aflux, allmap);
	...
	for (int iz=irmin; iz<=irmax; ++iz) // rings partially in the disc
	{
		...
		//planck_assert(ysq>=0, "error in query_disc()");
		Real dphi=atan2(sqrt(ysq),x);
		in_ring_allskymap (nside_, iz, phi, dphi, vecx, vecy, vecz, radius / 2.0, weight_norm, aflux, allmap);
	}
	...
	/*------------------------------------------add particles------------------------------------------------------*/
		{
			...
			if( 2.0*angular_radius < theta0 ) {
				...
				if(Npix < 2) {
					...
					query_disc_calcflux(Nside, theta, phi, 2.0*angular_radius, vecx, vecy, vecz, weight_norm, fluxes, allskymap);
					...
					for(int j=0;j<Npix;j++) {
						...
						Real _t, _p;
						pix2ang_ring(Nside, this_pix, &_t, &_p);
						Real this_vecx = sin(_t) * cos(_p);
						Real this_vecy = sin(_t) * sin(_p);
						Real this_vecz = cos(_t);
						Real d2 = acos( this_vecx * vecx + this_vecy * vecy + this_vecz * vecz) / angular_radius;
